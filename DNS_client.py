...
def parse_resource(start):
	...
	try:
		type = reverse_type_dict[("0x%0.2X" % message[start+n_len+1])]
	except KeyError:
		type = '0'
	ans_len = int.from_bytes(message[(start+8+n_len):(start+10+n_len)], byteorder='big')
	...
	if type == 'A':
		if (type_str == 'A'):
			...
			print(name + '  internet address = ' + parse_ip(ans_len, start+10+n_len))
	...
	if type == 'AAAA':
		if (type_str == 'AAAA'):
			...
			print(name + '  internet address = ' + parse_ipv6(ans_len, start+10+n_len))
	if type == 'NS':
	...
	if type == 'SOA':
		...
		print('\t' + 'serial = ' + str(int.from_bytes(message[(newstart):(newstart+4)], byteorder='big')))
		print('\t' + 'refresh = ' + str(int.from_bytes(message[(newstart+4):(newstart+8)], byteorder='big')))
		print('\t' + 'retry = ' + str(int.from_bytes(message[(newstart+8):(newstart+12)], byteorder='big')))
		print('\t' + 'expire = ' + str(int.from_bytes(message[(newstart+12):(newstart+16)], byteorder='big')))
		print('\t' + 'default TTL = ' + str(int.from_bytes(message[(newstart+16):(newstart+20)], byteorder='big')))
		
	if type == 'PTR':
		print(name + '\t' + 'name = ' + parse_name(start+10+n_len)[0])
		
	if type == 'MX':
		pref = str(int.from_bytes(message[(start+10+n_len):(start+12+n_len)], byteorder='big'))
		print(name + '\t' + 'MX preference = ' + pref + ', mails exchanger = ' + parse_name(start+12+n_len)[0])
